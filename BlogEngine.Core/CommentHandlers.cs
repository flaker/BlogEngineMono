...
//                              select type)))
			...
			foreach (var type in
                codeAssemblies.Cast<Assembly>().Select(a => a.GetTypes()).SelectMany(
                    types => (types.Where(type => type.GetInterface("BlogEngine.Core.ICustomFilter") != null).Select(
                        type => new {type, found = dt.Rows.Cast<DataRow>().Any(row => row[0].ToString() == type.Name)}).
                                 Where(@t => !@t.found).Select(@t => @t.type))))
            {
